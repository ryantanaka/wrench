...
//    }
     ...
     * @param hostname: the hostname on which to start the service
     * @param hosts_in_network: the hosts running in the network
     * @param measurement_period: the time (in seconds) between measurements at a daemon (must be > 1sec)
     * @param noise: a random noise added to the measurement period to avoid weird synchronicity behaviors
     * @param plist: a property list ({} means "use all defaults")
    ...
    NetworkProximityService::NetworkProximityService(
      ...
      // Check arguments
      if (measurement_period <= 1.0) {
        throw std::invalid_argument("NetworkProximityService::NetworkProximityService(): invalid measurement_period argument");
      }
      if (noise < 0.0) {
        throw std::invalid_argument("NetworkProximityService::NetworkProximityService(): invalid noise argument");
      }
      ...
      for (it=this->hosts_in_network.begin();it!=this->hosts_in_network.end();it++){
        this->network_daemons.push_back(std::unique_ptr<NetworkProximityDaemon>(new NetworkProximityDaemon(*it,this->mailbox_name, this->getPropertyValueAsDouble(
                NetworkProximityServiceProperty::NETWORK_PROXIMITY_TRANSFER_MESSAGE_PAYLOAD), measurement_period, noise)));
      }
    }
