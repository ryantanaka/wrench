  ...
  */
    ...
    */
    ...
    void Simulation::dumpWorkflowExecutionJSON(Workflow *workflow, std::string file_path) {
        ...
        nlohmann::json workflowtask_data;
        ...
        for (auto const &task : tasks) {
            ...
            while (not execution_history.empty()) {
                auto task_execution = execution_history.top();
                ...
                task_execution_json["compute"]    = {{START, task_execution.computation_start}, {END, task_execution.computation_end}};
                task_execution_json["write"]      = {{START, task_execution.write_output_start},{END, task_execution.write_output_end}};

                task_execution_json["failed"]     = task_execution.task_failed;
                task_execution_json["terminated"] = task_execution.task_terminated;

                workflowtask_data.push_back(task_execution_json);
                execution_history.pop();
            }
        }

        std::ofstream output(file_path);
        output << std::setw(4) << workflowtask_data << std::endl;
        output.close();
    }
    ...
    void Simulation::dumpWorkflowGraphJSON(wrench::Workflow *workflow, std::string file_path) {
        ...
        for (const auto &task : workflow->getTasks()) {
            ...
            // then create the links from the current task to its children tasks (if any)
